...
{
    internal class TradeMeAuthenticationHandler : AuthenticationHandler<TradeMeAuthenticationOptions>
    {
        private static readonly DateTime Epoch = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
        private const string StateCookie = "__TradeMeState";

        private readonly HttpClient _httpClient;
        private readonly ILogger _logger;

        public TradeMeAuthenticationHandler(HttpClient httpClient, ILogger logger)
        {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                JObject user = await ObtainUserProfile(Options.ConsumerKey, Options.ConsumerSecret, accessToken, oauthVerifier);

                var context = new TradeMeAuthenticatedContext(Context, user, accessToken.Token, accessToken.TokenSecret);
                ...
                if (!String.IsNullOrEmpty(context.MemberId))
                {
                    context.Identity.AddClaim(new Claim(ClaimTypes.NameIdentifier, context.MemberId,
                ...
                if (!String.IsNullOrEmpty(context.MemberId))
                {
                    context.Identity.AddClaim(new Claim("urn:TradeMe:memberid", context.MemberId,
                        "http://www.w3.org/2001/XMLSchema#string", Options.AuthenticationType));
                }
                if (!String.IsNullOrEmpty(context.NickName))
                {
                    context.Identity.AddClaim(new Claim("urn:TradeMe:nickname", context.NickName,
                        "http://www.w3.org/2001/XMLSchema#string", Options.AuthenticationType));
                }
        ...
        protected override async Task ApplyResponseChallengeAsync()
        {
            ...
            if (challenge != null)
            {
                ...
                {
                    string tradeMeAuthenticationEndpoint = Options.Endpoints.AuthorizationEndpoint + "?oauth_token=" + requestToken.Token;
                    ...
                    Response.Headers.Set("Location", tradeMeAuthenticationEndpoint);
                }
                else
                {
        ...
        public async Task<bool> InvokeReturnPathAsync()
        {
            ...
            var context = new TradeMeReturnEndpointContext(Context, model)
            {
        ...
        {
            // http://developer.trademe.co.nz/api-overview/authentication/example-oauth-flow/
            ...
            {
                authorizationParts.Add("scope", string.Join(",", Options.Scope));
            }
            ...
            canonicalizedRequestBuilder.Append(Uri.EscapeDataString(Options.Endpoints.RequestTokenEndpoint));
            canonicalizedRequestBuilder.Append("&");
            ...
            var request = new HttpRequestMessage(HttpMethod.Post, Options.Endpoints.RequestTokenEndpoint);
        ...
        private async Task<AccessToken> ObtainAccessTokenAsync(string consumerKey, string consumerSecret, RequestToken token, string verifier)
        {
            ...
            canonicalizedRequestBuilder.Append(HttpMethod.Post.Method);
            canonicalizedRequestBuilder.Append("&");
            canonicalizedRequestBuilder.Append(Uri.EscapeDataString(Options.Endpoints.AccessTokenEndpoint));
            canonicalizedRequestBuilder.Append("&");
            canonicalizedRequestBuilder.Append(Uri.EscapeDataString(parameterString));
            ...
            var request = new HttpRequestMessage(HttpMethod.Post, Options.Endpoints.AccessTokenEndpoint);
            request.Headers.Add("Authorization", authorizationHeaderBuilder.ToString());
        ...
        private async Task<JObject> ObtainUserProfile(string consumerKey, string consumerSecret, AccessToken token, string verifier)
        {
            ...
            string nonce = Guid.NewGuid().ToString("N");
            string requestUrl = Options.Endpoints.UserProfileEndpoint;
